  ...
  const filtered = records
    .filter((record) => {
      if (activeTab === "overdue") {
        return record.fine && record.fine > 0;
      }
      return true;
    })
    .filter((record) => {
      const lower = query.toLowerCase();
      return `${record.user?.name} ${record.user?.email}`.toLowerCase().includes(lower);
    });
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-6 space-y-4">
          ...
          <div className="bg-white rounded shadow-sm p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            ...
            />
            <button onClick={loadRecords} className="px-3 py-2 border rounded text-sm">
              Refresh
            </button>
          </div>
